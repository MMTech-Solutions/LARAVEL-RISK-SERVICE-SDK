...
- **Accounts**: `getAnalyticsDailyDayTrades` — `GET /accounts/{account_id}/analytics/daily/{date_utc}/trades` with optional filters (`from_utc`, `to_utc`, `symbol`, `side`, `session`, `phase_id`).
- **Analytics DTOs**: `AnalyticsDailyDayTradesResponseItem`, `AnalyticsDailyTradeRowItem`.
...
- **Accounts**: `getAnalyticsProfitability` — `GET /accounts/{account_id}/analytics/profitability` with optional filters (`from_utc`, `to_utc`, `symbol`, `side`, `session`, `phase_id`). Map success `data` with existing `AnalyticsProfitabilitySliceItem`.

## [3.13.0] - 2026-08-14

### Added

- **Accounts**: `getAnalyticsEquityCurve` — `GET /accounts/{account_id}/analytics/equity-curve` with optional filters (`from_utc`, `to_utc`, `symbol`, `side`, `session`, `phase_id`, `marker_limit`). Map success `data` with existing `AnalyticsEquityCurveSliceItem`.

## [3.12.0] - 2026-08-14

### Added

- **Accounts**: `getAnalyticsOverview` — `GET /accounts/{account_id}/analytics/overview` with optional filters (`from_utc`, `to_utc`, `symbol`, `side`, `session`, `phase_id`). Map success `data` with existing `AnalyticsOverviewSliceItem`.
...
- **`RuleActionType`** enum — `close_all_positions`, `disable_trading` (Risk API rule `actions[].type`).
- **`RuleActionInputItem`** — typed action with `toArray()`; factories `closeAllPositions()` and `disableTrading(int $durationMs)`.
- **`RuleActionPayloadBuilder::toPayloadList()`** — serializes action list for request bodies.
- **`CreateRuleCommand`**, **`UpdateRuleCommand`**, **`ProvisionRuleCommand`** — optional `actions` parameter.
- **`RuleResponseItem::$actions`** — actions returned by the API.

## [3.9.0] - 2026-08-03

### Added

- **`BrokerCTraderSdkConfigItem`** — added `manager_proxy_host`, `manager_proxy_port`, `manager_plant_id`, `manager_environment_name` (CTrader Hybrid Manager API connection hints).
- **`BrokerSdkConfigInputItem`** — added the same four `manager_*` fields.
- **`BrokerSdkConfigAssembler::forCTrader()` / `toPayload()`** — map and serialize the new `manager_*` fields.
